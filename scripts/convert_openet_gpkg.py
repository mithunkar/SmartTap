...
MM_YY_UNITS_RE = re.compile(r"^(?P<metric>.+)_(?P<mm>\d{2})_(?P<yy>\d{2})_(?P<units>\w+)$") # e.g., ETa_11_84_in
...
def classify_time_columns(columns: List[str]) -> Tuple[List[str], List[str]]:
    ...
    non_time = []
    for c in columns:
        if (YEAR2_RE.match(c) or YYYYMM_RE.match(c) or YYYY_MM_RE.match(c) or 
            YYYY_DASH_MM_RE.match(c) or MM_YY_UNITS_RE.match(c)):
            time_cols.append(c)
        else:
            non_time.append(c)
...
def parse_time_col(col: str) -> Optional[Tuple[str, int, Optional[int]]]:
    ...
    """
    # Try MM_YY_units format first (e.g., ETa_11_84_in)
    m = MM_YY_UNITS_RE.match(col)
    if m:
